...
function toAstro(cMonth: number, cDay: number) {
...
function solar2lunar(y: number, m: string | number, d: number | undefined) { // 参数区间1900.1.31~2100.12.31
  // 年份限定、上限
  ...
  return {
    lYear: year, lMonth: month, lDay: day, Animal: getAnimal(year), IMonthCn: (isLeap ? '\u95f0' : '') + toChinaMonth(month), IDayCn: toChinaDay(day), cYear: y, cMonth: m, cDay: d, gzYear: gzY, gzMonth: gzM, gzDay: gzD, isToday, isLeap, nWeek, ncWeek: `\u661f\u671f${cWeek}`, isTerm, Term, astro,
  }
}
...
const calendarFormatter = {
  ...
  solar2lunar(y: number, m: string | number, d: number | undefined) { // 参数区间1900.1.31~2100.12.31
  ...
  lunar2solar(y: number, m: number, d: number, isLeapMonth: boolean) { // 参数区间1900.1.31~2100.12.1
