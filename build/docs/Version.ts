...
browserModules.forEach((module) => {
  ...
  const curModuleNum =
    entryContent.match(/export\s\{.*\}\sfrom\s'.*'/g)?.length || 0;
  hooksNum += curModuleNum;
});

const nodeModuleEntryPath = `${nodePath}/index.ts`;
const nodeModuleContent = readFileSync(nodeModuleEntryPath, {
  encoding: 'utf-8',
});
const nodeModuleNum =
  nodeModuleContent.match(/export\s\{.*\}\sfrom\s'.*'/g)?.length || 0;
...
let changelogNeedContent = changelogContent.slice(
  firstFindStrIndex + findStr.length + 1,
);
